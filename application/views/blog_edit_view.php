		...
		<div class="blue_container">
			<div class="container">
				<article class="blog_create blue_text_container">
					<h2>Edit entry #<?= $blog_data['id'] ?> in the blog!</h2>
					<? if(!empty($error_messages)){	?>
						<section class="form_errors">
							<h5>There were errors in updating to the blog, please check below:</h5>
							<ul>
								<?= $error_messages ?>
							</ul>
						</section>
					...
					<? }else{ ?>
						<h4>Update your submission below!</h4>
					<? } ?>
					<?= form_open($form_destination, array('class' => 'form-horizontal blog_create_form')) ?>
						<div class="control-group">
							...
							<div class="controls">
								<?= form_input(array('name' => 'title', 'id' => 'form_title', 'class' => 'span8', 'value' => ((set_value('title')) ? set_value('title') : $blog_data['title']))) ?>
							</div>
						</div>
						<div class="control-group">
						...
						<div class="form-actions">
							<?= form_button(array('name' => 'submit', 'class' => 'btn btn-primary', 'type' => 'submit', 'value' => 'true', 'content' => 'Update Application')) ?>
						</div>
					<?= form_close() ?>
				</article>
			</div>
